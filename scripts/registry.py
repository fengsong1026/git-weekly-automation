...
def cleanup_orphans() -> list:
    ...
    for project_path, entry in list(registry["installations"].items()):
        if not Path(project_path).is_dir():
            # Project directory is gone — clean up its plist files
            for task_name, task_info in entry.get("scheduled_tasks", {}).items():
                ...
                if plist_path and Path(plist_path).exists():
                    try:
                        ...
                        pass
                # Also try to unload from launchd
            ...
            # Remove the original hooksPath if this project set it
            if entry.get("hooks_path_set") and entry.get("original_hooks_path") is not None:
                ...
                # Only restore if the current hooksPath still points to the dead project
            ...
            del registry["installations"][project_path]
